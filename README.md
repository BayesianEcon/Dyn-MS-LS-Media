...
install all the required packages. Make also sure to change the working directory to your own path whenever 
...
Estimates the the MS-LS model on the dynamic network dataset.
Running time (*) > 20 hrs 
...
Generates a synthetic dataset and estimates the the MS-LS model.
Running time ~13 mins



The following R scripts files allow to plot the Figures reported in the main text:
...
* JASA_Simulation_02_results.R
Generates the figures of the sythetic data analysis (Figure 5, Figure D.1, Figure D.2, Figure D.3, Table D.1)
...
with a brief description of the main function and its signature.C++ is integrated in R via Rcpp.

* JASA_MS_LS_FE.cpp
The script contains the MCMC function to estimate the Bayesian MS-LS network model for a dynamic network.
The dynamic network is expected to have N nodes for each time t=1,...,Time and count weigheted edges.


INPUT

The MCMC function expects the use of the cloumns of 3 dataframes as main input: "EL_x", "EL_princ", "DBplane".
...
FUNCTION SIGNATURE
Here below a description of all the arguments of the function MCMC
...
- interp_eq = 0 implies that the MS-LS model is computed diregarding equation 3 (see the manuscript)
...
The code implements a time-varying cosine similarity between the language use by political parties
And the language used by news outlets on a daily basis. The output is the DBplane dataset for Italy.
Running time ~45 mins.

The code is also used to riproduce Figure F.1 and F.2. 
...
The script contains a function, namely MCMC, suitable for the static analysis (similar to main c++ script).
...
Nodes represent news outlets and the weight associated to each edge represents the number of "commenters" in common between
