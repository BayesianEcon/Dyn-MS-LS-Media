...
result = MCMC(princ_w = EL_princ$w,
              ...
              acc_zeta_a = 0.25 , 
              acc_zeta_b = 0.25 , 
              pivot =  which(pages_names == "ABC.es"), 
              sign = 1,
              interp_eq = interp_eq, 
              ms_eq = ms_eq,
              rg_eq = rg_eq,
...
if(interp_eq ==1 & ms_eq == 1){
  ...
  save(result, file = "Code/06-Dynamic/RESULT_SC_SPz_fe.RData")
  
  
}else if(interp_eq ==1 & ms_eq == 0){
  
  save(result, file = "Code/06-Dynamic/RESULT_SC_SPz_fe_no_ms.RData")
  
}else if(interp_eq ==0 & ms_eq == 1){
  
  save(result, file = "Code/06-Dynamic/RESULT_SC_SPz_fe_no_interp.RData")
  
}

################Plots########################

list.of.packages <- c("ggplot2", "RcppDist", "ggrepel", "tidyr", "dplyr", "data.table", "patchwork")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)
...
cor(pew_sp_b$X1, pew_sp_b$score_a)
plot(pew_sp_b$X1, pew_sp_b$score_a)
