...
result = MCMC(princ_w = EL_princ$w, #edge weights from the network triangle dataset
              ...
              prop_sd_phi = 6,  #proposal RWMH for the phi parameter
              ...
              pivot = which(pages_names == "l'Humanité"),  # pivot node (news outlet) to solve identification
...
save(result, file = paste0("Extended/RESULT_SC_FRz_fe_extended_K_",as.numeric(args[1]),"_D_",as.numeric(args[2]),".Rdata"))