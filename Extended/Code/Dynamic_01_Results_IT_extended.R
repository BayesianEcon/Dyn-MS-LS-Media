...
result = MCMC(princ_w = EL_princ$w, #edge weights from the network triangle dataset
              ...
              gamma_1 = 1,  #starting value - gamma_1
              ...
              pivot = which(pages_names == "Libero"),  # pivot node (news outlet) to solve identification
...
DIC<- -2*mean(ResLik[35000:45000]) +2*(likeBar - mean(ResLik[interval]) )
DIC

result$DIC<- DIC

save(result, file = paste0("Extended/RESULT_SC_ITz_fe_extended_K_",as.numeric(args[1]),"_D_",as.numeric(args[2]),".Rdata"))
