...
result = MCMC(princ_w = EL_princ$w, #edge weights from the network triangle dataset
              ...
              pivot = which(pages_names == "Bild"),  # pivot node (news outlet) to solve identification
...
save(result, file = paste0("Extended/RESULT_SC_DEz_fe_extended_K_",as.numeric(args[1]),"_D_",as.numeric(args[2]),".Rdata"))